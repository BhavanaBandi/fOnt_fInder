#!/usr/bin/env python3
"""
Training script for Model_M_75M: Hybrid CNN-ViT (~75M parameters)

Architecture: MBConv blocks + Vision Transformer
Features: FocalLoss for class imbalance, tqdm progress bars
...
from models.Model_M_75M import (
    HybridCNNViTFontDetector, FocalLoss, create_font_detector
)
...
def main():
    parser = argparse.ArgumentParser(description='Train Model_M_75M (Hybrid CNN-ViT)')
    ...
    parser.add_argument('--batch_size', type=int, default=32,
                        help='Batch size (32 for 16GB GPU)')
    ...
    print(f"\n{'='*70}")
    print(f"Training Model_M_75M (Hybrid CNN-ViT, ~75M params)")
    ...
    torch.save({
        ...
        'num_classes': num_classes,
        'args': vars(args)
    }, save_dir / 'model_M_75M_final.pth')
    
    # Save history
    with open(save_dir / 'model_M_75M_history.json', 'w') as f:
        json.dump(history, f, indent=2)
    
    print(f"\n{'='*70}")
    print(f"Training Complete!")
    print(f"Best Validation Accuracy: {best_acc:.2f}%")
    print(f"Model saved to: {save_dir / 'model_M_75M_best.pth'}")
    print(f"{'='*70}\n")
